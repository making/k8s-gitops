...
## ノード分散 (podAntiAffinity)

`externalTrafficPolicy: Local` の前提として、**各 haproxy pod は別ノードに居る
必要がある**。同じノードに 2 個寄ると片側のノードには local endpoint が無くなり、
NLB はその node にトラフィックを送らなくなる (片肺運転)。

`spec.template.spec.affinity.podAntiAffinity.requiredDuringSchedulingIgnoredDuringExecution`
を `topologyKey: kubernetes.io/hostname` で指定して、同じ app=haproxy ラベルが
ある node には schedule できないように強制している。

このため `strategy.rollingUpdate` は `maxSurge: 0 / maxUnavailable: 1` 必須:

- ノードが 2 台しかなく既に各ノードに pod が居るので、3 個目 (surge) は
  「別 node」を満たせず Pending になる。
- maxUnavailable: 1 ＋ surge: 0 で「先に 1 個落とす → 別 node に新しく立てる」
  という順序になる。

ロール中は瞬間的に ready pod が 1 個になるが、preStop sleep (60s) の間は
Terminating 中の pod も外部に対しては稼働しているので NLB は両 node 経由で
配信を続けられる。
